...
% kappa_t = linspace(10,100,201);
% kappa_t = [12 16 23.1 30 50];
kappa_t = 25;
t = 0;
u = 0;
for kappa = kappa_t
    kappa
    u = u+1;
    K_charater = kappa/lambda*c_0;
    % sigma_j = [0 0.02 0.0436  0.0628 0.08 ];
    sigma_j = linspace(0,0.12,301);
    % sigma_j = [0.012];
    j = 0;
    ...
    for sigma = sigma_j
        sigma
        j = j+1;
        S_charater = sigma/(lambda*c_0);
        eta_span = 0.00:0.0005:1; %% fraction η which characterize the area ratio between a spherical cap and a sphere.
        i = 0;
        F_tot = 0;
        for eta = eta_span
            i = i+1;
            ...
            if eta==0
                ...
                F_poly = (9/2)*(pi*R_0^2/xi^2)*(R_0/(2*sqrt(eta)))*(3*v^(1/2)/(xi*a^2))^(2/3)*((1+10*N*sqrt(eta)/3/R_0*(v*a^2/3/xi^2)^(1/3))^(1/5)-1);
                F_line = 2*pi*lambda*R_0*sqrt(1-eta);
        ...
        E{j} = F';
        leg{j}=strcat('\sigma=',num2str(sigma),'k_BT/nm^2');
        t = t+1;
        Results(t,:) = [kappa,K_charater,sigma,S_charater,eta_span(index_min),y,Budding,H_brush(index_min),Z_max(index_min),lambda,c_0,...
            xi,rho_g,N_p,N,a,R_0,brush];
    end
end
F_all = table(E{:});
eta_span = eta_span';
...
subplot(2,2,2);
plot(sigma_j,eta_min,'linewidth',2)
xlabel('\it\sigma');
ylabel('\it\eta_{min}')
...
xlim([0 16.1]);
ylim([-0.05 3.05]);
xlabel('\it\kappac_0/\lambda');
ylabel('\it\sigma/(\lambdac_0)')
box on